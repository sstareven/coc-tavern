...
` }),
  ...
  coc_lore: { name: '克苏鲁深渊档案馆', enabled: true, entries: {
    ...
    necronomicon: e({ name: '神话典籍', keys: '死灵之书, Necronomicon, 禁忌古籍, 魔法书, 无名祭祀书, 伊波恩之书, 妖蛆的秘密, 塞拉伊诺断章, 典籍', logic: 'AND_ANY', priority: 30, position: 1,
      content: '【神话典籍】世间流传着若干被严密封存的禁忌古籍。它们记载着不为常人所知的黑暗知识——据说阅读神话典籍可增长「克苏鲁神话」技能，但每一页都以理智值(SAN)为代价。多数人穷其一生也无缘得见真本。\n<% if (getvar(\'剧情.已解锁.接触禁书\') === \'true\' || getvar(\'剧情.已解锁.密大特殊馆藏\') === \'true\') { %>\n主要典籍：\n《死灵之书》——阿卜杜·阿尔哈兹莱德于公元730年所著，记载旧日支配者历史、宇宙真实构造、召唤仪式。密斯卡塔尼克大学藏有拉丁文残卷。\n《无名祭祀书》——冯·容茨所著，记载各地秘密教团的仪式。\n《伊波恩之书》——超波里亚时代的魔法典籍，含大量法术。\n《塞拉伊诺断章》——记载外星知识的碎片文献。\n《妖蛆的秘密》——路德维希·普林记载的黑暗魔法。\n阅读时间数周至数月不等，每本典籍提供的神话知识和SAN损失各不相同。\n<% } %>' }),
  ...
  coc_style: { name: '洛夫克拉夫特文风', enabled: true, entries: {
    style_progression: e({ name: '每回合剧情推进硬约束', keys: '文风, 叙事, 推进, 剧情', logic: 'AND_ANY', priority: 201, constant: true,
      content: '【每回合必须推进剧情·硬约束】\n你在讲一个不断前进的故事，不是写风景散文。每一回合的正文都必须让故事真正向前走一步，严禁只把当前场景再描述一遍、渲染一通气氛就结束的「水文」。\n本回合 leftContent 至少包含下列「推进点」之一（通常一到两项即可，不要硬塞）：\n- 揭示新信息或线索——调查员看到/听到/意识到此前不知道的事实\n- 发生新事件或变故——有人出现或离开、环境突变、意外打断、计划生变\n- 某个 NPC 采取行动、表态或暴露动机（通过言行而非心理独白）\n- 调查员处境或状态改变——位置移动、时间推进、受伤、得失物品、被察觉\n- 冲突或悬念升级——威胁逼近、抉择代价变重、暗线浮现新征兆\n- 对上一回合玩家行动给出明确具体的结果与后果，而非含糊带过\n要求：\n- 氛围、感官与心理描写是烘托推进的佐料，绝不能替代推进；一段华丽却什么都没发生的描写就是失败。\n- 即便玩家选择「等待/观察」等被动行动，世界也要自己动起来（时间流逝、暗线推进、NPC 行动），不能原地踏步。\n- 已经描述过的场景不要重复铺陈，把笔墨集中在「这一回合新发生了什么」。\n- 推进须自然可信，服务于当前叙事弧线与暗线，不是为快而快地乱塞事件。' }),
    style_main: e({ name: '文风主规则', keys: '文风, 叙事, 描写', logic: 'AND_ANY', priority: 200, constant: true,
      content: '【守秘人叙事风格】\n视角：全知叙述者，保留神秘感\n时态：过去时\n句式：长句为主，大量使用破折号和分号构建复杂从句\n节奏：缓慢而沉重的叙述节奏，长句叠加形成窒息般的压迫感，偶尔以短句制造突然的恐惧冲击\n对话占比：对话与描写并重——对话常用来推进信息、暴露 NPC 动机与制造冲突；氛围与心理描写服务于剧情推进，不可只渲染气氛而不推进任何事\n描写重点：环境氛围、建筑与地貌细节、异常感官体验(光影/气味/声音)、心理恐惧递进、不可名状之物的暗示\n规则：\n- 单次正文(leftContent主叙事)篇幅严格控制在800字以内，宁可凝练克制，不要冗长堆砌；超长会稀释压迫感\n- 行动引导(rightContent右页)控制在200字以内，简短点出当前处境与抉择，不与正文重复铺陈\n- 恐怖元素通过暗示和间接描写传达，而非直接展示\n- NPC对话应带有地域口音和时代特色(1920年代新英格兰)\n- 检定结果融入叙事而非机械报告\n- 空间与移动逻辑：角色绝不凭空瞬移到某地。前往任何不在当前位置的地点，都必须通过步行或合理交通工具(火车/汽车/马车/船/徒步等)抵达，并简述移动过程、路线与耗时；地点变更须与上一场景在地理、距离与时间上自洽，跨城镇的远行应占用相应的天数或时辰\n- 保持洛夫克拉夫特式的学术考据风格叙述' }),
