...
describe('useDiceStore 暂存检定记录（剧情真正推进后才入 history）', () => {
  ...
  it('commitPending(explicitPage) 显式传入页号——覆盖 pageIndex fallback (Fix #9 append off-by-one)', () => {
    // 生产路径: useChatPipeline 在 appendPage 之后调 commitPending，此刻 autoFlipForward 还没
    // 跑，store.pageIndex 仍是旧页 N-1。如果走 fallback(pageIndex+1)=N，与同回合
    // diceFromInput 用的 checkPage=N+1 错位。显式传 baseIdx+2 才能两路同源。
    useBookStore.setState({ pageIndex: 2 }); // 旧页 N-1=2 → 新页号应为 4
    useDiceStore.getState().stashRecord(rec({ skill: '听力', page: 3 }));
    useDiceStore.getState().commitPending(4); // 显式 baseIdx+2=4
    expect(useDiceStore.getState().history[0].page).toBe(4);
  });

  it('commitPending(任意页号) 即使与 pageIndex 完全无关，也以显式值为准', () => {
    useBookStore.setState({ pageIndex: 0 });
    useDiceStore.getState().stashRecord(rec({ skill: 'X', page: 1, time: 1 }));
    useDiceStore.getState().stashRecord(rec({ skill: 'Y', page: 1, time: 2 }));
    useDiceStore.getState().commitPending(7);
    const hist = useDiceStore.getState().history;
    expect(hist.map((r) => r.page)).toEqual([7, 7]);
  });

  it('commitPending(undefined) 等价于不传——走 pageIndex+1 fallback (向后兼容)', () => {
    useBookStore.setState({ pageIndex: 4 });
    useDiceStore.getState().stashRecord(rec({ skill: '听力', page: 1 }));
    useDiceStore.getState().commitPending(undefined);
    expect(useDiceStore.getState().history[0].page).toBe(5);
  });
