...
| `useChatStore.ts` | 会话管理、预设加载 | localStorage (`coc_chat_v1`) ✅ 已持久化 |
...
- **`require()` 懒加载** — `useVariableStore` 和 `useTavernHelperStore` 用 `require()` 访问 `useCharSheetStore`。绕过 Tree-shaking 和类型检查。剩余约 2 处。
