...
export const COC_KP_PRESET: ChatPreset = {
  ...
  promptItems: [
    ...
    { id: 'pi_writing_format', name: '写作格式规范', kind: 'prompt', role: 'system', trigger: [], position: 'relative', depth: 0, order: 85, content: '【写作格式】严格使用中文标点符号，禁止使用英文标点。规则如下：\n- 句号用「。」不用「.」，逗号用「，」不用「,」，冒号用「：」不用「:」，分号用「；」不用「;」\n- 问号用「？」不用「?」，感叹号用「！」不用「!」，省略号用「……」不用「...」\n- 人物对话用中文直角引号：「对话内容」，引号内嵌套用『内层引号』\n- 破折号用「——」(两个em dash)，不用「--」或单个「—」\n- 书名号用「《》」，篇名号用「〈〉」\n- 顿号用「、」分隔并列词语\n- 数字与单位之间不加空格，中英文之间不加空格\n- 段落之间用空行分隔，不要使用缩进', enabled: true, _library: true },
