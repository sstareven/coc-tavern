...
/** 角色卡 8 段背景拼接器 — 与 CharCreator handleConfirm 同款 markdown 格式。
 *  PeopleTab 编辑独立字段后同步调此函数重建 sheet.description。
 *  缺省字段用 CharCreator 同款文学占位文案。 */
export interface CharSheetBackgroundFields {
  description?: string;
  beliefs?: string;
  significantPeople?: string;
  meaningfulLocations?: string;
  ...
  injuries?: string;
  backgroundFears?: string;
  /** personality 字段(NPC 行为/语气)合并进【特质】段,不再独立 sheet.personality */
  personality?: string;
}

export function buildCharSheetDescription(fields: CharSheetBackgroundFields): string {
  // 【特质】段 = traits + personality 拼接(traits 优先,personality 在后,换行分隔)
  const traitsParts: string[] = [];
  if (fields.traits?.trim()) traitsParts.push(fields.traits.trim());
  if (fields.personality?.trim()) traitsParts.push(fields.personality.trim());
  const traitsContent = traitsParts.length > 0 ? traitsParts.join('\n') : '沉默寡言，行踪不定。';
  const parts: string[] = [];
  parts.push(`【个人描述】\n${fields.description?.trim() || '此人的过往如同被墨水浸染的旧档案，所有记录都已模糊不清。'}`);
  parts.push(`【思想/信念】\n${fields.beliefs?.trim() || '信念栏是空白的——或许什么都不相信，又或许信念过于危险，不宜写下。'}`);
  parts.push(`【重要之人】\n${fields.significantPeople?.trim() || '没有任何人被列为重要联系人。这意味着孤独，或者意味着保护。'}`);
  parts.push(`【重要场所】\n${fields.meaningfulLocations?.trim() || '档案中未记录任何意义非凡之地。也许那些地方已经不复存在了。'}`);
  parts.push(`【珍贵之物】\n${fields.treasuredPossessions?.trim() || '此人似乎没有任何牵挂之物——或者说，那些珍贵的东西早已失去。'}`);
  parts.push(`【特质】\n${traitsContent}`);
  parts.push(`【伤口/伤痕】\n${fields.injuries?.trim() || '表面上看不出明显伤痕，但谁知道衣领下藏着什么。'}`);
  parts.push(`【恐惧症/狂躁症】\n${fields.backgroundFears?.trim() || '未记录在案。但每个调查员都有不愿面对的东西。'}`);
  return parts.join('\n\n');
}

function buildCharSheetDescriptionLegacy(input: MakeNpcInput): string {
  // 旧入口保留以避免大量调用点改动;实际转发到新签名
  return buildCharSheetDescription({
    description: input.description,
    beliefs: input.beliefs,
    significantPeople: input.significantPeople,
    meaningfulLocations: input.meaningfulLocations,
    treasuredPossessions: input.treasuredPossessions,
    traits: input.traits,
...
export function makeNpc(input: MakeNpcInput): ScenarioCharacter {
  ...
  return {
    ...
    npcAttrs: {
      ...
      // 8 段背景独立字段同步存到 npcAttrs,让 PeopleTab 编辑器读到拆开值
      description: input.description,
      beliefs: input.beliefs,
      significantPeople: input.significantPeople,
      meaningfulLocations: input.meaningfulLocations,
