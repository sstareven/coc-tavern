...
const SYSTEM_PROMPT = [
  ...
  '  "leftHeader": "序章",',
  '  "leftContent": string,         // 200-450 字,第三人称限知视角(主语统一为「调查员」,不要写「我」或「你」),克苏鲁神话基调,不剧透暗线;前 1/3 段必须先交代起因(a)与任务(b),然后才能氛围渲染',
  '  "rightHeader": string,         // 8 字内,如「现场」「邀请」',
  '  "rightContent": string,        // 100-200 字,承接序章把镜头落到调查员当下抉择点,必须直接服务于 leftContent 已交代的第一任务,主语统一为「调查员」',
  ...
  '',
  '【规则】',
  '- text 是给玩家看的选项摘要(8-16 字),action 是玩家选后被注入 prompt 的第三人称行动陈述,主语统一为「调查员」(严禁写「我」或「你」)。',
