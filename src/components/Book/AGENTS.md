...
**9 files, ~1730 lines.** CSS 3D 双页翻页系统。自定义缓动数学、`transform-style: preserve-3d`、`backface-visibility`。不使用 Framer Motion（`PageFlip.tsx` 已删除）。

## OVERVIEW

故事书是应用的核心视觉组件。左页显示叙事内容（带滚动发光粒子），右页显示交互选项和骰子检定结果。翻页动画完全由 CSS 3D transform 驱动（三次贝塞尔缓动），通过 `usePageFlip` hook 编排状态机。与 `InputBar.tsx` 通过自定义 DOM 事件通信。`resolvePlayerValue.ts` 将 LLM 输出的属性/技能别名归一到角色卡精确键。

## WHERE TO LOOK

| Task | Location | Notes |
|------|----------|-------|
| 翻页编排器 | `Storybook.tsx` | 513 lines，书签标签、目录弹窗、翻页状态机 |
| 右页（交互/骰子） | `RightPage.tsx` | 473 lines，骰子检定解析、选项按钮、代码块渲染 |
| 滚动粒子 | `ScrollParticles.tsx` | 119 lines，`useScrollGlow()` hook，滚动速度驱动边缘发光 |
| CSS 3D 翻页引擎 | `PageFlip3D.tsx` | 143 lines，`CSSFlipPage`/`FadingPage`/`AppearPage` 组件 |
| 场景状态栏 | `StatusBar.tsx` | 119 lines，页码/Token 显示 |
| 玩家值归一 | `resolvePlayerValue.ts` | 54 lines，属性/技能别名 → 角色卡精确键（+ `resolvePlayerValue.test.ts` 23 tests） |
| 左页（叙事） | `LeftPage.tsx` | 95 lines，滚动粒子发光效果，`@keyframes particleFloatUp/Down` |
...
- **`PageFlip.tsx`（Framer Motion 版）已删除** — 翻页统一由 `PageFlip3D.tsx`（CSS 3D）实现
