...
| Task | Location | Notes |
|------|----------|-------|
| 设置面板入口 | `SettingsPanel.tsx` | 侧边栏 tab 切换，AnimatePresence 过渡（839 lines ⚠️） |
...
| 预设编辑器 | `PresetEditor.tsx` | 采样参数 + Prompt items（671 lines） |
...
- **`DEFAULT_PRESET` 重复定义** — PresetEditor.tsx 和 PresetPanel.tsx 各有一份 ⚠️（L7-15、L8-40）
