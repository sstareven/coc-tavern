...
import { rowStyle, labelStyle, numInputStyle, Toggle, HelpIcon, CategoryBar } from './_shared';
import { IconRefresh } from '../Layout/TabIcons';
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, borderBottom: '1px solid rgba(196,168,85,0.18)', paddingBottom: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            fontSize: 'calc(11px * var(--system-ratio, 1))', padding: '4px 10px',
        ...
        >
          <IconRefresh size={12} /> 重置
        </button>
      </div>

      {/* Extension & Generation */}
      <CategoryBar label="扩展与生成" first />
      <Row checked={pt.enabled} onChange={(v) => set({ enabled: v })}
        label="是否启用扩展" help="全局开关" />
      <Row checked={pt.generateEnabled} onChange={(v) => set({ generateEnabled: v })}
        label="处理生成内容" help="生成全局开关" />
      <Row checked={pt.generateLoaderEnabled} onChange={(v) => set({ generateLoaderEnabled: v })}
        label="生成时注入 GENERATE 世界书条目" help="允许世界书条目的 GENERATE:BEFORE 和 GENERATE:AFTER 注入到生成提示词中" />
      <Row checked={pt.injectLoaderEnabled} onChange={(v) => set({ injectLoaderEnabled: v })}
        label="生成时注入 @INJECT 世界书条目" help="允许世界书条目的 @INJECT 注入到生成提示词中。Contributed by kanon0914" />

      {/* Message & Render */}
      <CategoryBar label="消息与渲染处理" />
      <Row checked={pt.renderEnabled} onChange={(v) => set({ renderEnabled: v })}
        label="处理保存消息" help="楼层全局开关" />
      <Row checked={pt.renderLoaderEnabled} onChange={(v) => set({ renderLoaderEnabled: v })}
        label="渲染楼层时注入 RENDER 世界书条目" help="允许世界书条目的 RENDER:BEFORE 和 RENDER:AFTER 注入到楼层消息中。仅影响显示内容，不影响生成。一般用来显示状态栏。" />
      <Row checked={pt.codeBlocksEnabled} onChange={(v) => set({ codeBlocksEnabled: v })}
        label="处理代码块" help="允许在处理楼层消息时对 pre 块进行模板处理" />
      <Row checked={pt.permanentEvaluation} onChange={(v) => set({ permanentEvaluation: v })}
        label="处理原始消息内容" help="先对原始消息进行模板处理，会修改原始消息内容。可以避免重复处理楼层消息，建议开启。" />
      <Row checked={pt.filterChatMessage} onChange={(v) => set({ filterChatMessage: v })}
        label="生成时忽略楼层消息处理" help="在生成前将楼层消息里的模板语句隐藏，不参与生成。" />
      <div style={rowStyle}>
        <span style={labelStyle}>
          楼层处理最大深度
          <HelpIcon text="仅处理深度小于此值的楼层。-1 表示无限制。" />
        </span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <input type="number" value={pt.chatDepth}
            onChange={(e) => set({ chatDepth: Number(e.target.value) })}
            min={-1} max={100} style={numInputStyle} />
        </div>
      </div>

      {/* World Book & Control */}
      <CategoryBar label="世界书与运行控制" />
      <Row checked={pt.autosaveEnabled} onChange={(v) => set({ autosaveEnabled: v })}
        label="自动保存变量更新" help="模板处理结束后立即保存。一般不需要开启，酒馆也会自行保存的。" />
      <Row checked={pt.preloadWorldinfo} onChange={(v) => set({ preloadWorldinfo: v })}
        label="立即加载世界书" help="打开角色卡时立即加载并处理世界书。一般用来初始化变量和 define。" />
      <Row checked={pt.withContextDisabled} onChange={(v) => set({ withContextDisabled: v })}
        label="禁用 with 语句块" help="用来解决 getvar is not defined 的问题。开启会导致 define 和 const 冲突。" />
      <Row checked={pt.debugEnabled} onChange={(v) => set({ debugEnabled: v })}
        label="控制台显示详细信息" help="控制台显示详细日志。日志非常多。" />
      <Row checked={pt.invertEnabled} onChange={(v) => set({ invertEnabled: v })}
        label="GENERATE/RENDER/INJECT 条目禁用视为启用" help="旧设定兼容模式。世界书中的相应条目禁用时视为启用。" />
      <Row checked={pt.compileWorkers} onChange={(v) => set({ compileWorkers: v })}
        label="后台编译" help="使用 Web Workers 在后台编译，避免页面卡顿。" />
      <Row checked={pt.sandbox} onChange={(v) => set({ sandbox: v })}
        label="环境隔离" help="在 iframe 中执行代码，更安全，但性能下降。" />

      {/* Cache */}
      <CategoryBar label="缓存设置（实验性）" />
      <div style={{ fontSize: 'calc(10px * var(--system-ratio, 1))', color: 'var(--ink-faded)', fontFamily: 'var(--font-ui)', marginBottom: 8 }}>
        模板编译缓存，可以提速。目前存在一些问题。
      </div>
      <div style={rowStyle}>
        <span style={labelStyle}>
          缓存
          <HelpIcon text="模板编译缓存，可以提速性能。目前存在一些问题。" />
        </span>
      ...
      <div style={rowStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <input type="number" value={pt.cacheSize}
            onChange={(e) => set({ cacheSize: Number(e.target.value) })}
            min={0} max={512} step={8} style={numInputStyle} />
        </div>
      </div>
      <div style={rowStyle}>
        <span style={labelStyle}>
          缓存 Hash 函数
          <HelpIcon text="缓存 Hash 算法。不同算法性能会有差异。当前值：h32，还有 h64。" />
        </span>
...
function Row({ checked, onChange, label, help }: { checked: boolean; onChange: (v: boolean) => void; label: string; help: string }) {
  return (
    <div style={rowStyle}>
      <span style={labelStyle}>
        {label}
...
const miniSelect: CSSProperties = {
  padding: '4px 8px', border: '1px solid var(--brass)', borderRadius: 3,
