...
- **纯计算层** (`variables`, `regex-engine`, `prompt-assembler`, `token-counter`, `stream-parser`, `context-manager`, `format-converter`, `dice-engine`, `coc-rules`, `coc-data`, `post-processor`, `mvu-extractor`, `format-instruction`) — 不导入 stores，零副作用
- **胶水层** (`character-variables`, `context-builder`, `llm-response-parser`, `macro-engine`, `tavern-helper-macros`, `th-script-engine`) — 导入 stores
