...
**37 源文件 (+test/support).** 纯计算模块 + store-glue 模块 + 骰子引擎 + MVU ZOD 变量系统分层。组件不直接引用引擎内部，通过 stores 和 hooks 间接消费。
...
| **MVU ZOD 变量系统** | `mvu-jsonpatch.ts` `mvu-charsheet-redirect.ts` `mvu-var-access.ts` `mvu-flatten.ts` `mvu-format.ts` `mvu-initial-statdata.ts` | JSON Patch(replace/delta/insert/remove/move) over statData 嵌套树;`调查员.*` 重定向角色卡(单源真理,非 statData 叶子);`readVar/writeVar` 共享 getvar/setvar;`flattenStatData` 扁平化供宏;`formatStatDataYaml` 供 `{{format_message_variable::stat_data}}` 宏;`createInitialStatData` 角色创建种子。statData 树存 `useVariableStore`,blob 持久化于 `sessionLifecycle`(`__statData__` 行) |
...
| 关键词词典注入 | `keyword-injection.ts` | `buildKeywordInjection()` 混合策略：最近 N 页常驻 + 老词按文本匹配，包 LoreEntry 注入 prompt |
| 扩展运行时桥接 | `extension-runtime.ts` | `extensionsToScripts()` 把启用扩展(内联 code)转 TH 脚本，复用 th-script-engine 沙箱执行 |
...
- **纯计算层** (`variables`, `regex-engine`, `prompt-assembler`, `token-counter`, `stream-parser`, `context-manager`, `format-converter`, `dice-engine`, `coc-rules`, `coc-data`, `post-processor`, `mvu-extractor`, `format-instruction`, `rewrite-instruction`, `unified-macro-engine`, `choice-match`, `hidden-roll`, `parse-dice-input`, `worldinfo-scope`, `keyword-injection`, `extension-runtime`) — 不导入 stores，零副作用
- **胶水层** (`character-variables`, `context-builder`, `llm-response-parser`, `th-script-engine`, `rpm-limiter`, `slash-commands`) — 导入 stores（`slash-commands` 顶层 import `useVariableStore`/`useTavernHelperStore`；`rpm-limiter` 读 `useSettingsStore` 的 RPM 配置）
- **混合层** (`ejs-template`) — 内部使用 `require()` 懒加载 stores，防止循环引用
- **LRU 缓存** — `RegexProvider` 和 `ejs-template` 使用 `Map` 实现 LRU 驱逐
- **命名导出** — 无 `export default`，与项目全局一致
- **懒加载 stores** — `ejs-template.ts` 内部用 `require()` 懒加载 stores（防循环引用）；`slash-commands.ts` 已改顶层 ESM import（原 require 在纯 ESM 浏览器运行时恒抛 ReferenceError 并被 catch 吞掉，致 /var /set /thvar 等命令永久失效）
