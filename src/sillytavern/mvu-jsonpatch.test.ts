...
describe('schema 校验 — replace 越界拒绝 / delta 双界饱和·单界拒绝 / 枚举拒绝', () => {
  const schema = {
    rules: {
  ...
  it('delta 单边界字段越界仍被拒绝（HP min-only -100 → reject + range，触发自纠）', () => {
    const tree: Record<string, unknown> = { hp: 10 };
  ...
  it('delta 双界字段越界饱和到上界（小时 20+10 → 夹到 max 23）', () => {
  ...
  it('delta 双界字段下界饱和（小时 5-10 → 夹到 min 0）', () => {
    const tree: Record<string, unknown> = { 世界: { 时间: { 小时: 5 } } };
    const errors = applyMvuPatchCollect(tree, [{ op: 'delta', path: '/世界/时间/小时', value: -10 }], { schema });
    expect((tree as any).世界.时间.小时).toBe(0);
    expect(errors).toHaveLength(0);
  });

  it('delta VWD 元组双界字段越界饱和且保留描述串', () => {
    const tree: Record<string, unknown> = { 世界: { 时间: { 小时: [20, '钟点'] } } };
    const errors = applyMvuPatchCollect(tree, [{ op: 'delta', path: '/世界/时间/小时', value: 10 }], { schema });
    expect((tree as any).世界.时间.小时).toEqual([23, '钟点']);
    expect(errors).toHaveLength(0);
  });
