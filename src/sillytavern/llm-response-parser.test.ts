...
describe('parseLlmResponse', () => {
  ...
  describe('物品叙事一致性硬执行', () => {
    ...
    it('解析顶层 badEnding 字段为字符串', () => {
      const json = '{"leftHeader":"书房","leftContent":"你走进书房。","rightHeader":"行动","rightContent":"接下来？","choices":[{"num":"I","text":"离开","action":"离开"}],"badEnding":"调查员最终被献祭，唤醒沉睡的旧日支配者。"}';
      const r = parseLlmResponse(json);
      expect(r!.badEnding).toContain('旧日支配者');
    });

    it('无 badEnding 字段时为 undefined', () => {
      const json = '{"leftHeader":"书房","leftContent":"你走进书房。","rightHeader":"行动","rightContent":"接下来？","choices":[{"num":"I","text":"离开","action":"离开"}]}';
      const r = parseLlmResponse(json);
      expect(r!.badEnding).toBeUndefined();
    });
...
describe('stripMvu — 孤立 <kw>/</kw> 兜底剥除（LLM 偶发漏写开/闭标签）', () => {
  ...
  // 回归(2026-06-10):用户报告流式生成时正文露出 `</k` — 半截 kw 标签兜底
  it('剥末尾未闭合的 `</k`(LLM 截断/中转站丢字符)', () => {
    expect(stripMvu('调查员推开门</k')).toBe('调查员推开门');
  });
  it('剥末尾未闭合的 `</kw`(无 `>`)', () => {
    expect(stripMvu('调查员推开门</kw')).toBe('调查员推开门');
  });
  it('剥末尾未闭合的 `<kw`(无 `>`)', () => {
    expect(stripMvu('调查员推开门<kw')).toBe('调查员推开门');
  });
  it('剥末尾 `<kw 部分关键词`(带文字但无 `>`)', () => {
    expect(stripMvu('调查员推开门<kw 阿卡姆')).toBe('调查员推开门');
  });
});
