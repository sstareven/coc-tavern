...
describe('applyCharsheetRedirect — 幸运钳制 0~99', () => {
  it('replace 越界上限夹到 99', () => {
    expect(applyCharsheetRedirect(sheet(), '调查员.幸运', 'replace', 150)?.secondary.luck).toBe(99);
  });
  it('delta 越界下限夹到 0', () => {
    expect(applyCharsheetRedirect(sheet(), '调查员.幸运', 'delta', -100)?.secondary.luck).toBe(0);
  });
});
