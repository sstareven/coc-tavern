...
describe('applyCharsheetRedirect — 技能写入键归一(全角括号/专精)', () => {
  it('全角括号「射击（手枪）」归一为半角键「射击(手枪)」(不造孤儿键)', () => {
    const next = applyCharsheetRedirect(sheet(), '调查员.技能.射击（手枪）', 'replace', 50);
    expect(next?.sheet.skills['射击(手枪)']).toEqual({ base: 0, current: 50, ticked: false });
  ...
  it('裸名唯一前缀命中已有专精：写「射击」落到唯一的「射击(手枪)」', () => {
    const base = { ...sheet(), skills: { '射击(手枪)': { base: 20, current: 40 } } } as CharacterSheet;
    const next = applyCharsheetRedirect(base, '调查员.技能.射击', 'delta', 5);
    expect(next?.sheet.skills['射击(手枪)'].current).toBe(45);
    expect(next?.sheet.skills['射击']).toBeUndefined();
  });
  it('裸名前缀歧义(多个同前缀专精)：回落规范名、建裸键(记录当前 hits>1 行为)', () => {
    const base = { ...sheet(), skills: {
      '射击(手枪)': { base: 20, current: 40 },
      '射击(步枪)': { base: 25, current: 30 },
    } } as CharacterSheet;
    const next = applyCharsheetRedirect(base, '调查员.技能.射击', 'replace', 50);
    expect(next?.sheet.skills['射击']).toEqual({ base: 0, current: 50, ticked: false });
    expect(next?.sheet.skills['射击(手枪)'].current).toBe(40); // 既有专精不受影响
  });
});
