...
字段说明（每个 NPC 一项）：
- name：NPC 的【真名】，严禁加「船医·」「科学家·」「院长」「主帅」「督察」「俘虏祭司·」等职业/身份/称谓前缀。叙事里若写作「船医·诺艾米·阿尔贝拉」，name 只取「诺艾米·阿尔贝拉」；若写作「赫尔曼院长」，name 只取「赫尔曼」。叙事里完全没真名只有称谓（「那个修士」）的，跳过不要列。【绝对禁止】把调查员自己列进来。
...
3. name 字段不许带职业/身份前缀（系统按 name 严格匹配定位现有 NPC，前缀差异会创建出第二张卡）。
4. 只输出严格 JSON，不要任何额外文字、解释或代码围栏。空（叙事里其实无 NPC）则返回 { "npcUpdates": [] }：
{
  "npcUpdates": [
