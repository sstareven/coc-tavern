 ...
 * 每个都用各自不同的 system prompt 直接 fetch。它们之间 prompt 前缀完全不同 → DS 前缀缓存无法跨子调用复用。
 *
 * 优化：让所有子调用共用同一个【短而稳定】的 `SUBAGENT_SHARED_SYSTEM`，原各自 system 内容下沉到 user
 * 消息头部并加上 `[子任务: xxx]` 标签。同回合内多个子调用之间 messages[0] 字节完全相同 →
 * DS 前缀缓存可在子调用之间命中。同时跨回合（重复同样任务时）也能命中。
 *
 * Trade-off:
 *   - 收益：子调用之间共享 ~200 tokens 前缀缓存；多子调用回合（如开局生成坏结局+起始物品+地点元素+地图自检）
 *           能节省 ~600-1000 tokens 的 cache write
 *   - 风险：原 system 被通用化 → LLM 任务理解能力可能略下降；通过把任务说明置于 user 头部部分抵消
 *
 * 仅在 `dsCache.experimentalSubagentSharedSystem` 开关启用且模型属 DS 系列时生效；其他场景原样返回。
 */

import { useSettingsStore } from '../stores/useSettingsStore';
import { isDeepSeekSource } from './deepseek-cache-restructure';
 ...
 *
 * 启用条件（同时满足才包装）：
 *   - dsCache.experimentalSubagentSharedSystem === true
 *   - isDeepSeekSource(apiModel, targetSources) === true
 *
 * 包装规则：
...
export function wrapSubagentMessages(
  ...
  if (!dsCfg?.experimentalSubagentSharedSystem) return messages.slice();
  if (!isDeepSeekSource(s.apiModel, dsCfg.targetSources ?? 'deepseek,custom')) {
