...
【每回合必须推进剧情】每一回合的 leftContent 都要让故事真正前进，至少做到以下之一：揭示新信息或线索、发生新事件或变故、推动某个 NPC 行动或表态、改变调查员的处境或状态、升级冲突或悬念、给出上一回合玩家行动的明确具体后果。严禁只把当前场景再描述一遍、渲染气氛就结束的「水文」——氛围、感官与心理描写只是烘托推进的佐料，绝不能替代推进；即便玩家选择等待或观察，世界（时间流逝 / 暗线 / NPC）也应自行推进，不可原地踏步。已描述过的场景不要重复铺陈，把笔墨集中在「这一回合新发生了什么」。
