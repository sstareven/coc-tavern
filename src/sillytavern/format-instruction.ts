...
关键词高亮：在叙事文本中，用 \`<kw>...</kw>\` 标签包裹重要的人名、地名、物品、线索等关键词（如"步入<kw>阿卡姆</kw>的街道"、"桌上放着一本<kw>奈克特抄本</kw>"）。每段叙事至少包含2-5个关键词标记。注意：不要给变量补丁块加 \`<kw>\` 标签。

关键词词条：凡是你在叙事中用 \`<kw>...</kw>\` 标记的每一个关键词，都必须在 keywords 字段中提供简短解释（10-30字），不得遗漏——包括经典地名/术语等你认为「已知」的词也必须给出释义。keywords 字段的键须与叙事中 \`<kw>\` 标签内的文字完全一致。这是硬性要求：标记了却没有对应释义的关键词将无法显示悬浮提示，破坏体验。严禁用旧的 \`{{}}\` 双花括号语法包裹关键词——\`{{...}}\` 是变量宏专用语法（如 \`{{调查员.生命值.当前}}\`），用它包裹普通关键词会被宏引擎当变量错误替换，导致前缀缓存大幅失效。

【关键词标签·配对硬约束】\`<kw>\` 与 \`</kw>\` 必须严格成对出现——每个 \`<kw>\` 开标签必须紧跟匹配的 \`</kw>\` 闭标签，且两者中间只能包含一个完整关键词（不得跨段落、不得嵌套）。绝不允许出现孤立的 \`</kw>\` 或 \`<kw>\`（漏写开/闭标签）：孤立闭标签会让玩家看到 \`</kw>\` 字面字符串、破坏沉浸感。每次写 \`<kw>关键词</kw>\` 都要在脑中确认两端配齐，再继续往下写。
