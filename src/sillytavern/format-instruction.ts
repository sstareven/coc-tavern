...
- 首次解锁拯救路径：{"op":"replace","path":"/剧情/救援/路径/封印古神/已解锁","value":true} 同回合把全局状态推进到对峙：{"op":"replace","path":"/剧情/救援/全局状态","value":"对峙"}
- 推进拯救里程碑：{"op":"delta","path":"/剧情/救援/路径/封印古神/进度","value":25}（默认每里程碑 +25，引擎自动钳到 0..100；进度达 100 视为该路径完成）
- 锁定胜出路径（某条路径率先达 100 时）：{"op":"replace","path":"/剧情/救援/全局状态","value":"锁定"} 与 {"op":"replace","path":"/剧情/救援/胜出路径","value":"封印古神"}
...
【孤注一掷·后果升级·硬约束】当本回合 input 顶部的检定结果行带「(孤注一掷)」标记时（形如「[图书馆使用 d100=55/30 失败 (孤注一掷)]」），调查员是冒着重大风险强行二次掷骰：
- 若该次检定为【失败】或【大失败】，本回合叙事**必须**让调查员付出与情境相称的明显代价，至少其一——精神创伤(SAN 实际 delta)、肉体伤害(HP 实际 delta，可附带伤情状态条件)、关键线索/物证永久丢失或被破坏、行迹暴露(NPC 警觉/敌意上升、暗线 progress 跳进)、关键 NPC 状态恶化(死亡/逃脱/转敌对)、长期不利状态条件等。不可一笔带过、不可只渲染氛围却无实质后果。
- 若孤注一掷为【成功】或更佳，无需额外惩罚——它本就是冒险换来的奖励，叙事正常推进即可。
- 实质代价须在 leftContent/rightContent 叙事中具体描写并落账（SAN/HP/姿态/状态条件/暗线 progress 等用 JSONPatch 更新，物品丢失/损毁走 inventoryChanges，行迹/线索变动写进 darkThread/clues）。

【关键词高亮·硬约束】每一段叙事正文【必须】用 \`<kw>...</kw>\` 标签包裹 2-5 个关键词，不得低于 2 个——这是渲染层产出 hover tooltip 的【唯一信号】，不标就没高亮、玩家无法查阅释义。即便上文 chatHistory 老回合没标记或用旧语法 \`{{xxx}}\`，本回合也【必须立刻开始用 \`<kw>...</kw>\` 标】，不要模仿历史中的无标记叙事。

【应该标的词】① 地名（街道、城镇、山丘、河流、建筑物，如 <kw>阿卡姆</kw>/<kw>大学图书馆</kw>/<kw>密斯卡塔尼克河</kw>）；② 人名 + 姓氏 + 家族名（如 <kw>霍尔姆斯馆员</kw>/<kw>沃特雷家族</kw>）；③ 重要物品 / 线索 / 信件 / 武器 / 文物（如 <kw>奈克特抄本</kw>/<kw>密信</kw>/<kw>指节铜套</kw>）；④ 超自然现象 / 神话生物 / 邪教组织 / 仪式名（如 <kw>低语</kw>/<kw>夜魇</kw>/<kw>黑夜教团</kw>）；⑤ 调查员注意到的可疑环境特征（如 <kw>破布条</kw>/<kw>歪脖子榆树</kw>）。日常用语（雾、风、门、桌子、椅子）不必标——保留稀缺性才有意义。

【配对·硬约束】\`<kw>\` 与 \`</kw>\` 必须严格成对、中间放一个完整关键词、不得跨段落、不得嵌套、不得只写半边。

【关键词词条】凡叙事中用 \`<kw>\` 标记的关键词都必须在 keywords 字段提供 10-30 字解释，keys 与标签内文字完全一致，包括「已知」地名/术语也不得遗漏。严禁用旧的 \`{{}}\` 双花括号语法包裹关键词——\`{{...}}\` 是变量宏专用语法（如 \`{{调查员.生命值.当前}}\`），用它包裹普通关键词会被宏引擎当变量错误替换，导致前缀缓存大幅失效。
...
重要物品约束：调查员只能使用当前物品栏中实际拥有的物品。如果玩家试图使用不在物品栏中的物品，应在叙事中自然地说明该物品不在手边或身上没有，并引导玩家采取其他行动。环境中的物品需要先通过add获取后才能使用。【生成行动选项(choices)时同理】：凡涉及使用某件具体物品/工具的选项，该物品必须是上方「调查员随身物品」清单中实际拥有的、或本回合叙事中已明确出现在环境里可直接取用之物；严禁让选项使用调查员并未持有、且本回合叙事中也未出现的物品（例如随身物品里没有相机，就绝不能出现「使用相机拍照」之类选项）。若想引导玩家使用尚未拥有的环境物品，应先在叙事或选项中体现「拿取/捡起/找到」该物品，而非直接给出「使用」选项。
...
【对抗检定结果权威·必须遵守】系统回填的对抗结果形如 [XX对抗 玩家d100=A/B(状态1) vs 对手d100=C/D(状态2) → 胜利/失败/平局]。方括号内「→」之后的裁定（胜利/失败/平局）是【绝对权威的最终结果】，叙事必须无条件据此叙述对抗输赢——这是系统比较双方掷骰档次与技能值后得出的，不是单看任一方掷骰。规则：①裁定为「胜利」=调查员在此对抗中胜出、达成其意图，【即便玩家个人掷骰显示(失败)，最终仍是胜利】（说明调查员凭技能差距压过了同样失利的对手），绝不能因看到(失败)字样就写成调查员未能做到/落败；②裁定为「失败」=调查员落败或被压制；③裁定为「平局」=势均力敌、僵持不分胜负。严禁自行从玩家个人掷骰的成功/失败字样反推对抗输赢而与「→」后的裁定相悖。
...

{
  ...
  "clues": [
    {"name": "密信", "summary": "来源不明的信件，边缘有褐色斑点", "discoveryNarrative": "登记簿下藏着这封没有署名的信，纸张边缘的褐色斑点凑近闻有铁锈味——像是干涸的血。字迹潦草急促，末尾提到「子夜的地窖」，似乎写信人当时极度惊恐。", "relatedTo": ["大学图书馆", "地窖"], "tags": ["物证", "地点"]}
  ],
  "darkThread": {
  ...
  "mapUpdates": {
    "current": "大学图书馆",
    "newLocations": [
      {"name": "校门", "description": "密大正门，锈蚀的铁艺大门后是一条通往主楼的林荫道"},
...
【NPC name 字段·硬约束·防重复】npcUpdates[i].name 是名册条目的主键，系统按 name 字符串严格 trim 后逐字相等匹配定位现有 NPC——一个字、一个空格、一个标点的差异就会被当成「新 NPC」创建出第二张卡。两条铁律：①若该 NPC 在上方「重要NPC/过路NPC/在场角色行动·硬约束」区块或本会话历史中已出现，name 必须与该处出现的字符串完全一致（包括是否带「先生/夫人/船长/医师」等称谓、是否含「·」分隔，逐字复制即可，禁止简化、补全、翻译、加称谓、去称谓、改标点）。②若是【全新】首次登场的 NPC，name 取「最简短稳定的真名」（如「埃伦娜·武」「霍尔姆斯」），严禁加「科学家·」「船医·」「主帅」「院长」等职业/身份前缀——职业归 identity 字段、称谓归叙事正文，前缀放进 name 会让以后简称该角色时人格分裂成两条名册。一句话:name 永远等于已有名册中的字符串,或是不带头衔的真名,两者必居其一。
...
- newLocations：本回合首次出现的新地点数组，每项含 name 与 description（一句话简述）。生成任何新地点时都必须在此登记；凡 newEdges 或 current 引用到的地点，也必须在此登记并给出 description（不要出现只作为连线端点、却没有描述的地点）。
...
【战斗检定·交给战斗面板·硬约束】战斗已由专门的「即时战斗面板」逐回合结算。选项的 action 中【绝对不要】生成任何战斗类检定标记——包括但不限于 近战攻击/近身攻击/格斗/斗殴/擒拿/缴械/反击/闪避/射击命中/伤害骰 之类。若本回合剧情需要调查员发起攻击或搏斗，选项只写成【普通的攻击行动叙述】（text 与 action 均为纯叙事，如「拔出左轮射击邪教徒」「扑上去与看门人扭打」），系统会自动据此进入战斗面板逐回合处理。只有【非战斗】的技能检定（侦查/聆听/话术/图书馆/理智等）才放进 action 的检定标记。`;
...
/** 拯救世界模式指令：集齐 3 条关键线索后注入，剧情转入与暗线赛跑的终局。 */
export const SAVE_WORLD_INSTRUCTION = '【拯救世界·终局模式】调查员已集齐三条关键真相、看清了那场灾厄的全貌，时间无多。从此剧情进入与暗线赛跑的终局：①调查员的核心目标转为【阻止那场灾厄成形】，leftContent 叙事与全部行动选项都应服务于此（如何破坏仪式/封印源头/营救受害者/直面并挫败幕后黑手）；②营造分秒必争的紧迫感——暗线 progress 即倒计时，越接近 100 越危急、75+ 趋于不可逆；③请用 JSONPatch 把 /剧情/阶段 置为「高潮」（若尚未）。选项应聚焦于终结灾厄的果断行动，而非漫无目的的调查。';

/** 序章首幕目标点明：结合本局具体情境，向调查员自然点明核心使命（纯叙事，不加 JSON 字段）。 */
export const PROLOGUE_GOAL_INSTRUCTION = '【序章·点明使命】这是冒险的开端。请在本回合的 leftContent/rightContent 叙事中，结合本局的具体情境，自然地让调查员意识到自己的核心使命：必须深入调查、查明真相、收集足以破局的【关键线索】，以阻止一场正在悄然逼近的灾厄。点到为止、融入剧情与氛围，不要生硬说教，更不得剧透具体的幕后真相或坏结局。';
