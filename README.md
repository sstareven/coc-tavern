...
- **地点地图系统**：探索过的地点自动连成可视化网络（实线=可自由往返、虚线箭头=单向不可逆），双页浏览（左页地点清单+描述、右页连线图，当前所在地点高亮）；AI 的移动须沿已有连线，不可瞬移或逆行。
- **独立线索库**：物品浮层右页为「线索」，每条线索含详细「发现细节」叙事，记录调查员从中察觉的蛛丝马迹；物品统一为随身物品（已取消「装备/卸下」概念）。
...
当前测试套件 **588 个测试 / 40 个文件** 全部通过，覆盖宏引擎、LLM 响应解析、MVU JSON Patch、骰子引擎、COC 规则、行动补写、NPC / 地图 / 线索等核心逻辑。
